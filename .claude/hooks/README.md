...
| `autonomy-gate.mjs` | PreToolUse | `Bash\|Edit\|Write\|MultiEdit` | **Blocks** (exit 2) irreversible/outward actions **only when `CLAUDE_AUTONOMOUS=1`** (stands down interactively): push/merge `main`, deploy, destructive (`rm -rf`/docker prune/db drop/force-push), dep-install, PR create, `ssh`, and any write to the agent's OWN governance (`settings*.json`/`hooks/**`/`skills/**`/`memory/**`/`CLAUDE.md`/`.github/workflows/**`/`.env*`). Fail-closed. The SOLE gate for unattended runs — contract: `nuc-platform/09-autonomy-contract.md`. |
| `prior-art-check.mjs` | PostToolUse | `Edit\|Write\|MultiEdit` | **Advisory** (exit 2 = feedback, never blocks): when a plan with frontmatter `kind: feature`/`system-change` is flipped to `status: active` but its `## Prior art & sources` section has <2 external URLs, nudges to research prior art first (anti-bias / research-before-design). Opt-in via `kind:` — `fix`/`refactor`/`chore` plans never fire. |
