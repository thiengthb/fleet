...
try {
  ...
  if (tool === 'Bash') {
    ...
    const DENY = [
      { name: 'git push (stop-before-PR; pushing is gated)', re: /\bgit\s+push\b/ },
      {
        name: 'force push / history rewrite',
        re: /\bgit\s+push\b.*(--force|-f)\b|\bgit\s+rebase\b|\bgit\s+reset\s+--hard\b|\bgit\s+commit\s+--amend\b/,
      },
      { name: 'branch/merge mutation', re: /\bgit\s+merge\b|\bgit\s+branch\s+-D\b|\bgit\s+clean\s+-[a-z]*f/ },
      { name: 'PR create / merge', re: /\bgh\s+pr\s+(create|merge)\b/ },
