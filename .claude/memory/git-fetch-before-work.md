...
#1 + #2 from `nuc-platform/plans/b4b3-fixes-sandbox/` — only to find on push that `origin/main` was 12 commits ahead
...
Especially before:
- Reading `nuc-platform/plans/*-sandbox/` (sandbox dirs are deleted once installed — local stale = phantom work).
