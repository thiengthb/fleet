---
name: project-docs
description: Generate & sync a project's standard doc set (00-map AI-primer, decisions log, + the 01/02/03 set for web-apps) per platform/standards/documentation.md. Two modes — scaffold (create missing docs from the kind's reference app) and audit (detect code↔docs drift, read-only). Use when creating a project, when docs are missing/stale, for the docs step in /app-onboard, or when the user says "write docs" / "do the docs still match the code".
---

# Skill: Standard docs for a project (project-docs)

Generate and keep in sync the doc set per **`platform/standards/documentation.md`** (read it first — that's the
...
**Don't invent a different structure** from §3/§4/§5 of standards/documentation. If you find the standard needs to change →
...
2. **Generate `docs/00-map.md`** following exactly the 8-section skeleton in §4 of standards/documentation. Token-cheap rule: tables +
...
5. **Thin `CLAUDE.md`**: if there isn't one → create a short version (rules + project-specific invariants + a pointer "read
   ...
   before cutting.
6. **Update the index**: add/edit the project line in `platform/registries/knowledge-ledger.md §B`.
...
- The full file set is present per `kind` (table §3 of standards/documentation).
- `INVENTORY §0` has the project row; `registries/knowledge-ledger §B` has the pointer.
