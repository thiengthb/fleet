...
it). **Never** work on `main`. All commits are **local** (the gate blocks push).
...
## Step 5 — At a gate: PARK, don't push through
...
3. Emit a **digest** (Step 7) describing it for the human.
4. Stop the batch cleanly. A human approves later (or a supervised session does the gated step).
...
Emit a concise digest (this is what reaches the human via Discord in Layer B4; until then it goes to stdout / the run
log): **what I did** (steps ✓ + branch), **what's next**, **any gate awaiting you**, **budget note**.
...
never open a PR unattended · never edit your own governance (`.claude/settings*`, `hooks/**`, `skills/**`, `memory/**`,
any `CLAUDE.md`, CI, `.env*`). All of these are **propose-to-human**, never self-execute.
