...
<#
  app-env.ps1 - push a LOCAL key mirror file into a NUC app's .env over SSH, securely.
  Secrets travel keyboard -> local file -> ssh STDIN -> NUC; never on a command line, never through the agent chat.
  Idempotent upsert (existing key replaced, new key appended, others untouched) + auto force-recreate. Skill: /app-env.

  Usage:   ./app-env.ps1 <app> [-NoRestart] [-NucHost thien25@thienminiserver] [-EnvDir <dir>]
...
$remoteScript = Join-Path $here 'app-env-remote.sh'
...
Write-Host "[app-env] $(@($names).Count) key(s) -> ${NucHost}:/opt/apps/$App/.env" -ForegroundColor Cyan
Write-Host ("[app-env] keys: " + ($names -join ', '))
...
if ($LASTEXITCODE -ne 0) { Write-Error "[app-env] remote merge failed (exit $LASTEXITCODE)"; exit $LASTEXITCODE }
Write-Host "[app-env] done - env applied$(if (-not $NoRestart) { ' + container recreated' })." -ForegroundColor Green
