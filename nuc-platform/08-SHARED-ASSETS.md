...
| **Empty-state box** | The one "nothing here yet" placeholder: dashed border + centered muted text; `className` tunes padding and turns it into a stack (`flex flex-col items-center gap-4`) when it carries a CTA. ~15 lines, only dep = `cn` | `sakubun/components/empty-state.tsx` | `sakubun` (7 call sites consolidated 2026-07-19: history list+page, resources grammar/vocab tabs, review log, item detail, enrichment card) | copy-in → **ui-kit proposed** | **PATTERN — proposed for `ui-kit`** (built 1× but it de-duplicated 7 in-app copies; every app has empty surfaces. Agent must NOT install into ui-kit — needs user approval) |
