...
| **Sortable/filterable data table (SRS-item shape)** | Client-side filter + column sort (clickable headers, asc/desc indicator) + due-format toggle (relative ↔ exact `HH:MM DD/MM/YY`) + DataPagination w/ page-size | `sakubun/components/{items-table,vocab-table}.tsx` | `sakubun` ×2 (grammar + vocab, differ only by columns) | copy-in (→ ui-kit generic `<DataTable>` at the 3rd shape) | **DUPLICATED — extract candidate** (2× near-identical; the sort/due-toggle/paging logic is the same, only columns differ) |
