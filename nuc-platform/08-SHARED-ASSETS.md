...
| **Data pagination control** | Controlled pagination UI (`page`/`pageCount`/`onPageChange` + `…` window) + optional rows-per-page selector (`pageSize`/`onPageSizeChange`, 2026-07-12 → now deps `select`); client- or server-side paging | `ui-kit/registry/thiengthb/data-pagination.tsx` (`shadcn add data-pagination`) | `sakubun` (`/items`, `/vocab`, `/history`) | **REGISTRY (copy-in)** | REGISTRY (built 1×; canonical in ui-kit from the start per user directive — reuse platform-wide) |
| **DataTable (generic CRUD table, TanStack)** | Config-driven table on `@tanstack/react-table`: multi-column sort (Shift-click to add + priority badges), draggable column widths, show/hide columns, global search, an app **filter slot** + **action slot**, DataPagination — whole view (sort/visible-cols/page/size/search) persisted to sessionStorage + mirrored to the URL (widths session-only). App supplies `columns` (accessor + `cell` renderers → any custom cell incl. edit/delete row-actions) and pre-filters `data`. ⚠️ Needs `@tanstack/react-table` + shadcn `table`/`button`/`dropdown-menu`/`input` + `data-pagination` | `ui-kit/registry/thiengthb/data-table.tsx` (`shadcn add data-table`) | `sakubun` (`items-table` + `vocab-table` are now thin column-configs over it, 2026-07-18) | **REGISTRY (copy-in)** | REGISTRY (extracted from the 2× duplicated SRS-item tables — the prior "extract candidate", now realized) |
| **App sidebar (config-driven nav)** | Navigation sidebar on the shadcn `sidebar` primitive — collapses to an icon rail (⌘/Ctrl+B) + mobile Sheet + cookie-persisted state + per-item tooltip when collapsed; takes a `brand` + grouped `items` (lucide icons) + `footer` config so every app wires the SAME component. ⚠️ Next-only (`next/link` + `next/navigation`) | `ui-kit/registry/thiengthb/app-sidebar.tsx` (`shadcn add app-sidebar`) | `sakubun` (replaced the top navbar, 2026-07-12) | **REGISTRY (copy-in)** | REGISTRY (built 1×; canonical in ui-kit from the start per user directive — reuse platform-wide) |
...
| **Empty-state box** | The one "nothing here yet" placeholder: dashed border + icon/title/description/action, **or** free-form `children` for rich copy (inline `<code>`, an embedded dialog). `className` tunes padding and turns it into a stack (`flex flex-col items-center gap-4`). Only dep = `cn` + `lucide-react` | `ui-kit/registry/thiengthb/empty-state.tsx` | `todo` (8 sites) · `journal` · `sakubun` (7 sites) | **`ui-kit` copy-in** (`npx shadcn add empty-state`) | **SHARED.** 2026-07-19: sakubun had forked a `children`-only twin without noticing the registry entry → canonical gained an **additive** `children` slot (`title` now optional) and sakubun adopted it. Existing copies render identically, no re-sync needed. Default padding is `px-6 py-16` — a site relying on the old `p-8` must say so explicitly |
...
| **Breadcrumbs (config-driven trail)** | Generic renderer taking an ordered `Crumb[]` (each = label + href + lucide icon + description-as-tooltip); intermediate crumbs link, the last marks the current page (aria-current). App owns the route→crumb map (`sakubun/components/app-breadcrumbs.tsx`: server pages pass a string `section`, so icon components never cross the RSC boundary; detail pages get the parent chain e.g. Tài nguyên → Ngữ pháp → item). ⚠️ Next-only (`next/link`) + needs `TooltipProvider` + the `tooltip` primitive | `ui-kit/registry/thiengthb/breadcrumbs.tsx` (`shadcn add breadcrumbs`) | `sakubun` (all pages, 2026-07-18) | **REGISTRY (copy-in)** | REGISTRY (built 1×; canonical in ui-kit from the start per user directive — reuse platform-wide) |
| **Date / Time picker** | `date-picker` (Popover + shadcn `Calendar`, value = local `"YYYY-MM-DD"` string, optional `clearable`) + `time-picker` (typed `Input` + Popover of `step`′ presets, value = local `"HH:MM"` string, manual typing still valid). Both `w-full`, self-contained (inlined date/time helpers, no util dep); replace raw `<input type="date"/"time">` | `ui-kit/registry/thiengthb/{date-picker,time-picker}.tsx` (`shadcn add date-picker` / `time-picker`; deps: `date-fns`+`react-day-picker`+`calendar`/`popover` for date, `popover`+`scroll-area` for time) | `todo` (per its §12 UI convention), `sakubun` (settings "Giờ nhắc học" → TimePicker, 2026-07-17; date-picker installed but not yet used) | **REGISTRY (copy-in)** | REGISTRY (canonical in ui-kit from the start per user directive — reuse platform-wide) |
