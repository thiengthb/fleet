...
created: 2026-06-14
updated: 2026-06-15 # B4b.3 fixes INSTALLED: #1 leak + #2 (GATE)-count installed to live .claude/scripts (verified); #3 channel-authz applied + pushed to nuc-ops-bot (deploy); INVENTORY/auth-apps doc-synced; /nuc-set-env added to CLAUDE.md catalog; sandbox removed. B5 now UNBLOCKED. Open: finding #4 (worker reliability) → watch in B5
related:
  [
...
- [ ] B5 — **UNBLOCKED 2026-06-15** (finding #2 fixed: the orchestrator now crosses an approved gate on its own). Real unattended window on a low-risk plan, user supervising **from phone**: works → digest → park → approve from phone → PR step done in a supervised session. Test: full loop, zero gate crossed unattended.
