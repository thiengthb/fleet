...
related:
  [
    nuc-platform/10-idea-queue.md (idea-0011 — the candidate this analyzes),
    ...
    nuc-platform/log/README.md (the recall-tier day-log = the raw material the detector mines),
    nuc-platform/09-autonomy-contract.md (governance: agent proposes, human installs — the load-bearing constraint),
    ...
    nuc-platform/07-SKILL-CANDIDATES.md (external community-skill verdicts — the internal sibling queue),
  ]
---
...
- **Internal prior art (extend-don't-rebuild):** `/code-reuse`'s **rule of three** (1×build, 2×log, 3×extract) is the exact
  ...
  **recall-tier day-log** (`nuc-platform/log/`, shipped Phase 3) is the queryable record of "what happened repeatedly" the
  detector mines.
