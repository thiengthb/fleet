...
- [ ] 2 — **Author backup assets in-repo** (agent-doable): `nuc-platform/backup/restic-backup.sh` (dump PG → sqlite `.backup` → `restic backup` of all 8 volume paths + dumps → `rm` dumps → `restic forget --keep-daily 7 --keep-weekly 4 --keep-monthly 3 --prune`), `restic-backup.service` + `restic-backup.timer` + `restic-check.timer` templates, and an `OnFailure` notifier hook to nuc-monitor Discord. · Files: Create `nuc-platform/backup/*` · Test: `bash -n restic-backup.sh` clean; shellcheck clean
