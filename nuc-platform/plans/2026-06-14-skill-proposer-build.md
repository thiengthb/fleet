...
related:
  [
    nuc-platform/plans/2026-06-14-skill-proposer-induction-proposal.md (accepted RFC — design + sources + the why),
    nuc-platform/10-idea-queue.md (idea-0011 done),
    ...
    nuc-platform/log/README.md (recall-tier day-log = the raw material the detector mines),
    .claude/hooks/autonomy-gate.mjs (blocks .claude/skills/** → install is human-only; skill-proposals/ is T2),
    nuc-platform/09-autonomy-contract.md (propose-don't-install — the load-bearing constraint),
  ]
---
...
(`nuc-platform/skill-proposals/`) for a human to security-review + install — capturing Hermes' detect+draft value while
...
asked for — folded in as deferred Phase 2). Sandbox = `nuc-platform/skill-proposals/` (outside the `\.claude\/skills\/`
...
- **AC-1** — Given a process recurred ≥3× across the day-log/git, When `/skill-proposer` runs, Then it writes exactly one
  draft `SKILL.md` into `nuc-platform/skill-proposals/`, citing the ≥3 concrete instances — and NEVER into `.claude/skills/`.
- **AC-2** — Given an unattended run (`CLAUDE_AUTONOMOUS=1`), When the agent attempts a Write/Edit under `.claude/skills/`,
  Then `autonomy-gate.mjs` blocks it (T4); a Write under `nuc-platform/skill-proposals/` is allowed (T2).
...
- [x] C1 — Sandbox `nuc-platform/skill-proposals/README.md` (convention + proposal frontmatter + lifecycle: draft→human-review→install-by-move) + `_TEMPLATE.md`. Done (AC-1, AC-4 described).
...
- ~~Where do proposals live so the agent can write but not self-install?~~ Resolved: `nuc-platform/skill-proposals/` (T2), confirmed vs the gate's `\.claude\/skills\/` lock.
