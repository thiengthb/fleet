...
related:
  [
    ...
    nuc-platform/plans/2026-06-14-agent-os-evolution.md (parent — Phase 3 + RAG foundation),
    nuc-platform/06-knowledge-ledger.md (the cross-project index this complements),
    nuc-platform/05-documentation-standard.md (decisions.md = archival tier),
    ...
    nuc-platform/09-autonomy-contract.md (memory/** is governance-locked → log lives outside it),
  ]
---
...
- **If the autonomy worker can't write the log** → `.claude/memory/**` is governance-locked (T4). Mitigation: the log lives
  in `nuc-platform/log/` (outside the lock) so a T2 branch-local write is allowed; confirm `autonomy-gate.mjs` treats it as T2.
