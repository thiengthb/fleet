...
- [x] S3.1 — DONE 2026-06-19. Full-cycle ORDER was already wired (advance → graduate → enrol → C3 → reflect → watchdog); this adds the **retry cap = 3 then escalate** piece. New retry-cap registry in `auto-pilot-scheduled.ps1`: a per-target consecutive-failure streak (`retry-state.json` in `$LogDir`) keyed `advance:<plan>` / `graduate` / `enrol` / `reflect`. Each batch records its exit via `Set-RetryOutcome` (clean exit clears the streak); on the 3rd consecutive nonzero exit it escalates ONCE to Discord (`ask-cli report`) and BLOCKS that target (`Test-RetryBlocked` skips it) until a human clears the file/fixes the cause — other phases keep running. Complements the watchdog (which catches GLOBAL no-progress; this caps a SINGLE failing target so it never re-fires forever). A correctly-PARKED batch exits 0, so parking is NOT counted a failure (that's the watchdog's domain). C3 intentionally excluded (propose-only + its own once/day-on-success throttle). · Test: 7-scenario unit test (streak/escalate-once/clear-on-clean/independent-keys) ✓ + dry-run wiring ✓
...
- [x] S3.3 — FULLY PROVEN LIVE 2026-06-19. (i) **Enrol round-trip** (earlier): synthetic idea → graduation draft → `ASK-enrol-*` → Discord card → supervisor clicks enrol → RS256-signed answer → `ask-cli` verifies → next cycle arms (`status: active`+`auto_pilot: true`)+consumes; agent never self-arms. (ii) **Planning-Q&A round-trip (S1.2)** — synthetic idea-9997 with a deliberately-open design fork → graduation minted `ASK-graduate-9997a8` (3 options) → Discord → supervisor clicked **(a)** → signed answer → round-2 graduation verified+read (a) as DATA → finalized the draft plan reflecting (a), moved idea under `## Done`, consumed. (iii) **Single-transcript execute→wrap→retro (S2.x)** — a throwaway opted-in plan advanced (scratch file, box checked, plan→done) AND the reflection batch (wrap + retro-filed-nothing + `/idea sort` + Discord digest) both ran in ONE transcript. All synthetic residue cleaned (4 throwaway branches deleted, runtime state reset, dry-run opted-in=0). **The demo's payoff = a real bug (see S3.4c).** · Test: AC-2/AC-3/AC-6 ✓ (enrol + planning-Q&A + execute→reflect, all live)
- [x] S3.4 (NEW, from S3.3 findings) — Reliability fixes the live demo surfaced: **(a) DONE 2026-06-19** — `Invoke-GatePush` no longer swallows a failed `git push`. Root cause was deeper than the `catch {}`: in PowerShell `git push` FAILS WITHOUT THROWING (sets `$LASTEXITCODE`, no exception), so the catch never fired. Fix checks `$LASTEXITCODE`, retries ONCE after `pull --ff-only` (the concurrent-bot non-ff case), and on final failure emits a LOUD `WARNING: gate push FAILED` line + returns `$false` (local commit preserved → next cycle retries). Runtime-verified in a temp-git sandbox: clean→no-op·True, good remote→True+reached, broken remote→False+warning+commit preserved. **(b) DONE 2026-06-19** — `Invoke-GatePush` now distinguishes "not provisioned" (gates dir absent → silent no-op is correct) from "provisioned-but-broken" (dir EXISTS with pending `asks/`/`reports/` JSON but no `.git` → those can never reach Discord): the latter now WARNS + returns `$false` instead of a silent no-op. Plus the watchdog's UNCONSUMED detector is the runtime backstop that catches a worker parked on an unpublished ask. Also fixed a self-introduced `2>&1`-on-native-git slip (ledger #60 trap under Start-Transcript) → plain `| Out-Null` + `$LASTEXITCODE`. **(c) DONE 2026-06-19, found by the full-S3.3 live demo (branch `fix/gate-push-transcript-trap`, `3270250`)** — S3.4a's explicit-`$LASTEXITCODE` retry was UNREACHABLE in production: under `Start-Transcript` + `$ErrorActionPreference='Stop'`, a failed `git push`'s stderr becomes a TERMINATING `NativeCommandError` (ledger #60), crashing the wrapper at the push line before the retry ran (removing the old blanket `catch {}` in (a) exposed this). Also `pull --ff-only` cannot reconcile the real DIVERGENCE (bot pushed concurrently → local 1-ahead/1-behind). Fix: `$ErrorActionPreference='Continue'` inside `Invoke-GatePush` (native errors non-terminating → retry reachable) + retry via `pull --rebase` (+ `rebase --abort` fallback). Verified by REPRODUCING the trap in a sandbox under real `Start-Transcript`+`Stop` with a diverged origin (old crashes; patched survives+rebases+delivers, in-sync 0/0). · Files: `auto-pilot-scheduled.ps1` (gate-sync) · Test: temp-git sandbox 3-path ✓ + S3.4b glob ✓ + S3.4c trap-reproduction ✓
