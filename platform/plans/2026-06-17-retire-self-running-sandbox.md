...
related:
  ...
  - platform/plans/self-running-agent-sandbox/INSTALL.md
---
...
live counterparts. The draft sandbox `platform/plans/self-running-agent-sandbox/` is now mostly superseded and should
...
- [x] S3 (T2 edit) — `grep -rn 'self-running-agent-sandbox' platform .claude` and update/annotate every reference so
...
- Do NOT touch live governance (`.claude/settings*`, `skills/**`, `CLAUDE.md`, CI). Editing files UNDER
  `platform/plans/` and docs is normal T2 and allowed. The sandbox holds DRAFT governance copies, but removing them
