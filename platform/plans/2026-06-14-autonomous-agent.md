...
related:
  [
    platform/plans/2026-06-14-discord-control-plane.md (accepted RFC — B4 design),
...
- [x] B2 — Dumb orchestrator · `.claude/scripts/auto-pilot-run.ps1` + `.sh` (root `scripts/` is gitignored — control-plane repo only tracks `/.claude/` + `/platform/`) · loop fresh `claude -p` per batch (sets `CLAUDE_AUTONOMOUS=1`, no `--continue`, `--disallowedTools` defense-in-depth, never `--bare`); stop on no-progress/no-steps/cap; `--dry-run` · **Dry-loop verified** on both (8 unchecked, identical; PS fixed: ASCII-only + literal `(GATE)`). CLI flags validated via `claude --help`.
...
- [~] C3 — **Unattended proposer integration — DEFERRED 2026-06-20 (freeze decision; sandbox built, NOT installed — see R5).** Realized in the new
  ...
  `platform/plans/2026-06-16-self-running-agent.md`.
