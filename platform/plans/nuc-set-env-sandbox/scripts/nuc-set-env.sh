#!/usr/bin/env bash
# nuc-set-env.sh — Git Bash / Linux front-end (parity with nuc-set-env.ps1). Pushes ~/.nuc-env/<app>.env into the
# NUC app's .env over ssh; secrets via STDIN (never argv, never the agent chat). Idempotent upsert + force-recreate.
# Skill: /nuc-set-env.   usage: ./nuc-set-env.sh <app> [--no-restart]
...
[ -n "$app" ] || { echo "usage: nuc-set-env.sh <app> [--no-restart]" >&2; exit 1; }
...
remote_script="$here/nuc-set-env-remote.sh"
...
echo "[nuc-set-env] keys -> ${NUC_HOST}:/opt/apps/$app/.env : $names"
