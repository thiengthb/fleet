...
|--------|---------|------|--------|-----------|-------------|----------|-----|
| `platform` | **authentik** | `infra` (third party) | `nuc` | Central IdP (pinned image, manual update) | `thiengthb/authentik` (compose) | `fleet/projects/authentik` | `/opt/apps/authentik` |
| `platform` | **nuc-monitor** | `python-worker` | `nuc` | Monitors the NUC → Discord — python-worker reference | `thiengthb/nuc-monitor` | `fleet/projects/nuc-monitor` | `/opt/apps/nuc-monitor` |
| `platform` | **nuc-ops-bot** | `python-worker` (bot) | `nuc` | Discord ChatOps bot controlling the NUC | `thiengthb/nuc-ops-bot` | `fleet/projects/nuc-ops-bot` | `/opt/apps/nuc-ops-bot` |
| `platform` | **rulebook** | `node-service` | `local` | Delivers the platform's frontend rules to other projects without shipping the rulebook. **Primary path = a Claude Code plugin hook** (`plugins/rulebook-frontend`, offline, ~0 tokens/session). **Installed at USER scope, which is `~/.claude/` and does NOT travel with the repo — so the row says which machine:** Linux box since 2026-07-29; **`TNT-Laptop` (Windows) since 2026-08-01 — v0.3.0, from a `directory` marketplace pointing at `C:\project\fleet\rulebook` itself, so it tracks the working tree and a rule change takes effect after `npm run build:plugin` with no publish step (the cost: enforcement here breaks if the repo moves).** Verified in place, not assumed: a file-wide `rulebook-allow-file:` and a per-line `rulebook-allow` both honoured (exit 0), a genuinely dirty `.tsx` blocked (exit 2), a clean one and a non-UI file silent, an unreadable file loud (*"NOT checked. This is unknown, not clean."*), and **0 findings across all 333 UI files in the 4 apps**. Installing it is what exposed three CRLF defects (`cc496ee`): the checker's verdict depended on the checkout, so on this box it reported **11 false findings in 5 files** and would have blocked files whose exceptions were argued in writing. Also holds the MCP server it was first built as (`review_component` + `report_lesson`, port 3901, no auth) — **KEPT DELIBERATELY, zero consumers**, decided 2026-07-29 (proposal `2026-07-29-mcp-path-keep-or-retire.md`, Option A) on three retire-triggers now living as checkboxes in the build plan's check-in runbook Q2. Phase 3 verdict 2026-07-29 cancelled hosting: `target` stays `local`, `cloud` is NOT coming, `idea-0013` is not a prerequisite. **The MCP server must still never be exposed off-machine** (no auth exists) | `thiengthb/rulebook` (**public** — source + plugin; the service is not hosted) | `fleet/rulebook` | — |
...
| `product` | **sakubun** | `web-app` (Next) | `local` | **LOCAL-only** JA↔VI **grammar-driven** translation trainer (FSRS schedules grammar patterns only — vocab dropped 2026-07-07, learn in Anki) — Claude Desktop as tutor via MCP (port 3789). **AUTHED + multi-user since 2026-07-21** — better-auth session for web, a per-user revocable `McpKey` bearer for every machine surface; this cell said "no auth" until 2026-08-01, which was the pre-2026-07-21 rule. **Second machine surface 2026-08-01: a TERMINAL client** (`GET/POST /api/cli` on the SAME `McpKey` token; a full-screen Ink TUI plus scriptable commands covering the management half — generation/grading stay with Claude). NUC onboarding deferred (server down 2026-07) | `thiengthb/sakubun` (**private**; verified on GitHub 2026-07-30 — this cell said "no remote yet", the remote exists and was last pushed 2026-07-29) | `fleet/projects/sakubun` | — |
