...
| `platform` | **rulebook** | `node-service` | `local` | Delivers the platform's frontend rules to other projects without shipping the rulebook. **Primary path = a Claude Code plugin hook** (`plugins/rulebook-frontend`, offline, ~0 tokens/session). **Installed on ONE machine only — say which, because "user scope" is `~/.claude/` and does not travel with the repo:** installed 2026-07-29 on the **Linux box**; **measured 2026-08-01 on `TNT-Laptop` (Windows): NOT installed** — `claude plugin list` reports none, the `rulebook` marketplace is not configured (only `claude-plugins-official`), and `~/.claude/rulebook-usage.jsonl` does not exist. So on this box the primary path delivers nothing, and its usage log is silent for a third reason the check-in gate did not list. Also holds the MCP server it was first built as (`review_component` + `report_lesson`, port 3901, no auth) — **KEPT DELIBERATELY, zero consumers**, decided 2026-07-29 (proposal `2026-07-29-mcp-path-keep-or-retire.md`, Option A) on three retire-triggers now living as checkboxes in the build plan's check-in runbook Q2. Phase 3 verdict 2026-07-29 cancelled hosting: `target` stays `local`, `cloud` is NOT coming, `idea-0013` is not a prerequisite. **The MCP server must still never be exposed off-machine** (no auth exists) | `thiengthb/rulebook` (**public** — source + plugin; the service is not hosted) | `fleet/rulebook` | — |
