...
| `platform` | **rulebook** | `node-service` | `local` | Delivers the platform's frontend rules to other projects without shipping the rulebook. **Primary path = a Claude Code plugin hook** (`plugins/rulebook-frontend`, offline, ~0 tokens/session, installed at user scope 2026-07-29). Also holds the MCP server it was first built as (`review_component` + `report_lesson`, port 3901, no auth) — kept, local-only, **zero consumers**. Phase 3 verdict 2026-07-29 cancelled hosting: `target` stays `local`, `cloud` is NOT coming, `idea-0013` is not a prerequisite. **The MCP server must still never be exposed off-machine** (no auth exists) | `thiengthb/rulebook` (**public** — source + plugin; the service is not hosted) | `fleet/rulebook` | — |
