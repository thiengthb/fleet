...
| `platform` | **rulebook** | `node-service` | `local` | Serves the platform rulebook to other projects as **verdicts, not text** (MCP tool `review_component`, port 3901, no auth yet). Added 2026-07-29 — idea-0023 Phase 1. **Do NOT expose off-machine before `idea-0013`** (`@thiengthb/mcp-auth`); `target` becomes `cloud` only at Phase 4 | (no remote yet) | `fleet/rulebook` | — |
